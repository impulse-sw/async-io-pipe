...
When you're running some programs from `std::process::Command`, you need to coherently collect data from `stdout`/`stderr` streams, because some programs write some text in `stdout`, some in `stderr`, and with no order, you won't know the correct sequence of output data. `async-io-pipe` solves this problem by using non-blocking UNIX anonymous pipes ([`std::io::PipeWriter`](https://doc.rust-lang.org/stable/std/io/struct.PipeWriter.html), [`std::io::PipeReader`](https://doc.rust-lang.org/stable/std/io/struct.PipeReader.html) - added to Rust since v1.87.0).
